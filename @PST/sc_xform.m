function [T] = sc_xform(area,nmach_a,mac_con)
% Syntax   : [T] = sc_xform(area,nmach_a)
%
% Purpose  : To construct transformation from nominal electromechanical variables to "slow inter-area" and "fast intra-area" variables.
%
...
for r=1:num_area,             % cycle thru all areas
  if nmach_a(r) == 1          % single machine area
    ...
    Ha = sum(w_H);
    for i = 1:num_mach
      T1(r,mac_list(i))=mac_con(mac_list(i),3).*mac_con(mac_list(i),16)/Ha;                   
    end
    for i = 2:num_mach
      T2_count = T2_count+1;
