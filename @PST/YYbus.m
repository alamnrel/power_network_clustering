function     [Y,nSW,nPV,nPQ,SB] = YYbus(bus,line)
% Syntax:    [Y,nSW,nPV,nPQ,SB] = YYbus(bus,line) 
%
...
% Date:      March 1991
%
% Modified by I. Tyuryukanov to model transformer phase shifts 
% and turns ratios as being applied to the "from" side of the 
% transformer branch.
% ************************************************************
global bus_int;
...
for i = 1:nline
  ...
  % See MATPOWER (e.g. MATPOWER 7.0) User Manual about assumptions on 
  % phase shifter admittance matrices (IT 23May2020):
  tps = tap_ratio*exp(jay*phase_shift*pi/180);
  Y(from_int,to_int) = Y(from_int,to_int) - y(i)/conj(tps);
  Y(to_int,from_int) = Y(to_int,from_int) - y(i)/(tps);
  Y(from_int,from_int) = Y(from_int,from_int) + (y(i) + jay*chrg(i)/2)/(conj(tps)*(tps));  			   
  Y(to_int,to_int) = Y(to_int,to_int) + y(i) + jay*chrg(i)/2;  					
  % % sparse matrix formulation (original J.H. Chow code - IT)
  % j1(1,1) = from_int; j2(1,1) = to_int;
  % w(1,1) = - y(i)/(tps);
  % j1(2,1) = to_int; j2(2,1) = from_int;
  % w(2,1) = - y(i)/conj(tps);
  % j1(3,1) = from_int; j2(3,1) = from_int;
  % w(3,1) = (y(i) + jay*chrg(i)/2)/(conj(tps)*(tps));
...
% convert Y bus to sparse storage
%[i,j,y] = find(Y);
%Y = sparse(i,j,y,nbus,nbus);
%disp('Check sparse matrix Y') 
%keyboard
